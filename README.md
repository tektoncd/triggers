...
- [`EventListener`](docs/eventlisteners.md) - Connects `TriggerBindings` and `TriggerTemplates` into an [addressable](https://github.com/knative/eventing/blob/master/docs/spec/interfaces.md) endpoint (the event sink) with. When hit, it validates the event if `validate` task is provided. Then, it uses extracted event parameters from each `TriggeringBinding` to instantiate resources specified in the corresponding `TriggerTemplates`.
