...
When sending a hook protected by a secret, GitHub includes an `X-Hub-Signature-256` object in the header.
GitLab secret tokens use `X-GitLab-Token`. GitLab signing tokens (GitLab 19+) use
`webhook-id`, `webhook-timestamp`, and `webhook-signature`.
You may see `no X-Hub-Signature-256 header set`, `no X-GitLab-Token header set`,
and/or `no webhook-signature header set` errors in your logs in one of the following scenarios:
