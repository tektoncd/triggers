...
import (
	...
	interceptorinformer "github.com/tektoncd/triggers/pkg/client/injection/informers/triggers/v1alpha1/interceptortype"
...
var (
	github = &triggersv1.InterceptorType{
	...
	cel = &triggersv1.InterceptorType{
		ObjectMeta: metav1.ObjectMeta{
			Name: "cel",
		},
		Spec: triggersv1.InterceptorTypeSpec{
	...
	bitbucket = &triggersv1.InterceptorType{
		ObjectMeta: metav1.ObjectMeta{
			Name: "bitbucket",
		},
		Spec: triggersv1.InterceptorTypeSpec{
...
func getSinkAssets(t *testing.T, resources test.Resources, elName string, webhookInterceptor http.Handler) (Sink, *fakedynamic.FakeDynamicClient) {
	...
	r := Sink{
		...
		InterceptorTypeLister:       interceptorinformer.Get(ctx).Lister(),
	}
...
func TestHandleEvent(t *testing.T) {
	...
	tests := []struct {
		...
		resources: test.Resources{
			...
			InterceptorTypes: []*triggersv1.InterceptorType{github, cel},
			Triggers: []*triggersv1.Trigger{{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "git-clone-trigger",
					Namespace: namespace,
		...
		resources: test.Resources{
			...
			InterceptorTypes: []*triggersv1.InterceptorType{bitbucket},
			Triggers: []*triggersv1.Trigger{{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "git-clone-trigger",
					Namespace: namespace,
...
func TestHandleEvent_AuthOverride(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.userVal, func(t *testing.T) {
			...
			resources := test.Resources{
				InterceptorTypes: []*triggersv1.InterceptorType{github},
...
func TestExecuteInterceptor_NotContinue(t *testing.T) {
	resources := test.Resources{
		InterceptorTypes: []*triggersv1.InterceptorType{cel},
	}
