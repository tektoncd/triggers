...
func getSinkAssets(t *testing.T, resources test.Resources, elName string, webhookInterceptor http.Handler) (Sink, *fakedynamic.FakeDynamicClient) {
	...
	httpClient := intercepttest.SetupInterceptors(t, clients.Kube, logger.Sugar(), webhookInterceptor)
...
func TestExecuteInterceptor_Sequential(t *testing.T) {
	logger := zaptest.NewLogger(t)
	httpClient := intercepttest.SetupInterceptors(t, nil, logger.Sugar(), &sequentialInterceptor{})
...
func TestExecuteInterceptor_error(t *testing.T) {
	...
	r.Handle("/", si)
	httpClient := intercepttest.SetupInterceptors(t, nil, logger.Sugar(), r)
...
func TestExecuteInterceptor_ExtensionChaining(t *testing.T) {
	...
	resp, _, iresp, err := s.ExecuteInterceptors(trigger, req, []byte(body), s.Logger, eventID, nil)
	if err != nil {
		t.Fatalf("executeInterceptors: %v", err)
	}
