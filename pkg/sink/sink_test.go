...
func getSinkAssets(t *testing.T, resources test.Resources, elName string, auth AuthOverride) (Sink, *fakedynamic.FakeDynamicClient) {
	...
	coreInterceptors, err := server.NewWithCoreInterceptors(clients.Kube, logger.Sugar())
	if err != nil {
		t.Fatalf("failed to initialize core interceptors: %v", err)
	}
	srv := httptest.NewServer(coreInterceptors)
	t.Cleanup(func() {
		srv.Close()
	})
...
func TestHandleEventWithInterceptors(t *testing.T) {
	...
	sigHeader, err := test.HMACHeader("secret", eventBody)
	if err != nil {
		t.Fatalf("failed to generate Signature header: %v", err)
	}
	req.Header.Add("X-Hub-Signature", sigHeader)
...
func TestExecuteInterceptor_NotContinue(t *testing.T) {
	s, _ := getSinkAssets(t, test.Resources{}, "el-name", DefaultAuthOverride{})
	trigger := triggersv1.Trigger{
		Spec: triggersv1.TriggerSpec{
			Interceptors: []*triggersv1.EventInterceptor{{
				CEL: &triggersv1.CELInterceptor{
...
func TestExecuteInterceptor_ExtensionChaining(t *testing.T) {
	...
	// Setup a httptest Server that handles both core interceptors and the echoInterceptor
	// We use the echoInterceptor service name to distinguish which handler serves the request
	coreInterceptors, err := server.NewWithCoreInterceptors(clients.Kube, logger)
	if err != nil {
		t.Fatalf("failed to initialize core interceptors: %v", err)
	}
	...
	match := func(r *http.Request, _ *mux.RouteMatch) bool {
		return strings.Contains(r.Host, webhookInterceptorName)
	}
	r := mux.NewRouter()
	echoServer := &echoInterceptor{}
	r.MatcherFunc(match).Handler(echoServer)
	r.PathPrefix("/").Handler(coreInterceptors)
