...
func TestRecordRecordMetrics(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			s.recordMetrics(&StatusRecorder{Status: 200}, time.Duration(1000))
			metricstest.CheckDistributionData(t, "http_duration_seconds", test.expectedTags, 1, float64(test.expectedDuration), float64(test.expectedDuration))
