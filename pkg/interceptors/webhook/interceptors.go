...
func makeRequest(client *http.Client, request *http.Request) ([]byte, http.Header, error) {
	...
	if err != nil {
		...
		return nil, responseHeader, xerrors.Errorf("Failed to proxy request to interceptor: %w", err.Error())
	}
	defer resp.Body.Close()

	respBody, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, responseHeader, xerrors.Errorf("Failed to parse response body")
	}

	// Wrap error and return
	if resp.StatusCode != http.StatusOK {
		return nil, responseHeader, xerrors.Errorf("Request rejected; status: %s; message: %s", resp.Status, respBody)
	}
