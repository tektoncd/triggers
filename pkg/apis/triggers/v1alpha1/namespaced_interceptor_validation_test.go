...
func TestNamespacedInterceptorValidate_OnDelete(t *testing.T) {
	ci := triggersv1.NamespacedInterceptor{
		ObjectMeta: metav1.ObjectMeta{
			Name: "github",
		},
		Spec: triggersv1.NamespacedInterceptorSpec{
	...
	if err != nil {
		t.Errorf("NamespacedInterceptor.Validate() on Delete expected no error, but got one, NamespacedInterceptor: %v, error: %v", ci, err)
	}
}

func TestNamespacedInterceptorValidate(t *testing.T) {
	tests := []struct {
		name                  string
		namespacedInterceptor triggersv1.NamespacedInterceptor
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.want.Error(), got.Error()); diff != "" {
				t.Fatalf("NamespacedInterceptor.Validate() error: %s", diff)
			}
		})
	}
}
